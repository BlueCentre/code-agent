...
def test_config_defaults_only():
    ...
    assert config.auto_approve_native_commands == DEFAULT_CONFIG["auto_approve_native_commands"]
...
def test_config_file_only(mock_config_file, reset_config_cache):
    ...
    assert config.native_command_allowlist == mock_config_file["native_command_allowlist"]
...
def test_config_boolean_conversion_from_env(reset_config_cache):
    ...
    for env_str, expected_bool in env_cases.items():
        ...
        env_vars = {
            "CODE_AGENT_AUTO_APPROVE_EDITS": env_str
        }
        ...
        with patch("os.path.exists", return_value=False), \
            ...
            assert config.auto_approve_edits is expected_bool, \
...
def test_config_validation_rules():
    ...
    with patch("builtins.open", mock_open(read_data=yaml.dump(risky_config))), \
         patch("os.path.exists", return_value=True), \
         pytest.raises(ValueError) as exc_info:
        get_config()
