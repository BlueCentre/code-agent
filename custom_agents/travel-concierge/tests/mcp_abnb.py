...
async def async_main(question):
    ...
    session = session_service.create_session(state={}, app_name="travel-concierge", user_id="traveler0115")
    ...
    events_async = runner.run_async(session_id=session.id, user_id="traveler0115", new_message=content)
    ...
    async for event in events_async:
        ...
        function_calls = [e.function_call for e in event.content.parts if e.function_call]
        function_responses = [e.function_response for e in event.content.parts if e.function_response]
