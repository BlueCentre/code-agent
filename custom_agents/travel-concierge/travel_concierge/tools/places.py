...
class PlacesService:
    ...
    def _check_key(self):
        if not hasattr(self, "places_api_key") or not self.places_api_key:  # Either it doesn't exist or is None.
    ...
    def get_photo_urls(self, photos: List[Dict[str, Any]], maxwidth: int = 400) -> List[str]:
        ...
        for photo in photos:
            photo_url = (
                f"https://maps.googleapis.com/maps/api/place/photo?maxwidth={maxwidth}&photoreference={photo['photo_reference']}&key={self.places_api_key}"
            )
