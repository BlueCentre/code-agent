...
def parse_as_origin(origin_json: Dict[str, Any]):
    ...
    match origin_json["event_type"]:
        ...
        case "home":
            return (
                origin_json.get("local_prefer_mode") + " from " + origin_json.get("address", ""),
                "any time",
            )
        case _:
...
def parse_as_destin(destin_json: Dict[str, Any]):
    ...
    match destin_json["event_type"]:
        ...
        case "home":
            return (
                destin_json.get("local_prefer_mode") + " to " + destin_json.get("address", ""),
                "any time",
            )
        case _:
...
def find_segment(profile: Dict[str, Any], itinerary: Dict[str, Any], current_datetime: str):
    ...
    for day in itinerary.get("days", []):
        ...
        for event in day["events"]:
            ...
            print(event["event_type"], event_date, current_date, event_time, current_time)
...
def transit_coordination(readonly_context: ReadonlyContext):
    ...
    travel_from, travel_to, leave_by, arrive_by = find_segment(profile, itinerary, current_datetime)
