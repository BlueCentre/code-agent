...
def send_message(resource_id: str, message: str) -> None:
    ...
    remote_agent = agent_engines.get(resource_id)
    session = remote_agent.create_session(user_id="traveler0115")  # Optionally can provide initial states: state=initial_state
...
def main(argv: list[str]) -> None:
    ...
    initial_states_path = FLAGS.initial_states_path if FLAGS.initial_states_path else os.getenv("TRAVEL_CONCIERGE_SCENARIO")
    env_vars["TRAVEL_CONCIERGE_SCENARIO"] = initial_states_path
    map_key = FLAGS.initial_states_path if FLAGS.initial_states_path else os.getenv("GOOGLE_PLACES_API_KEY")
